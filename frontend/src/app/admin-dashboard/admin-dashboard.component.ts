...
export class AdminDashboardComponent implements OnInit {
  ...
  @ViewChild('universityUpsertModal') universityUpsertModal!: TemplateRef<any>;
  ...
  private performResetPassword(id: number): void {
    this.adminService.resetUserPassword(id).subscribe({
      next: (res: any) => {
        ...
        this.pendingResetId = null;
        setTimeout(() => (this.resetSuccessMessage = null), 5000);
      },
      error: (err) => {
        console.error('Failed to reset password', err);
        this.resetErrorMessage = err?.error?.message ?? 'Failed to reset password';
        this.pendingResetId = null;
        setTimeout(() => (this.resetErrorMessage = null), 5000);
      },
    });
  }
  ...
  openCreateUniversity(): void {
    ...
    this.modalService.open(this.universityUpsertModal, { centered: true, size: 'lg' });
  }

  openEditUniversity(u: any): void {
    this.editing = true;
    this.selectedUniversityId = u?.id ?? null;
  ...
  saveUniversity(modalRef: NgbModalRef): void {
    if (!this.uniForm.name || !this.uniForm.name.trim()) return;
    ...
    const req$ =
      this.editing && this.selectedUniversityId
    ...
    req$.subscribe({
      ...
      error: (err) => {
        ...
        alert(err?.error?.message ?? 'Failed to save university');
      },
    });
  }
  ...
  confirmDeleteUniversity(id: number): void {
    ...
    const modalRef: NgbModalRef = this.modalService.open(this.confirmUniversityDeleteModal, {
      centered: true,
    });
  ...
  private deleteUniversity(id: number): void {
  ...
  private loadUniversitiesOnly(): void {
    this.adminService.getUniversities().subscribe({
      next: (list: any[]) => (this.universities = list ?? []),
