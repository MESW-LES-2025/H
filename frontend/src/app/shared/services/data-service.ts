...
export class DataService {
  ...
  loadFilterLists(): void {
    forkJoin({
      areas: this.http
        .get<AreasOfStudy[]>(`${this.baseUrl}/api/area-of-study`)
        .pipe(map((areas) => areas.map((area) => area.name))),
      languages: this.http.get<string[]>(
        `${this.baseUrl}/api/courses/languages`,
      ),
      countries: this.http.get<string[]>(
        `${this.baseUrl}/api/university/countries`,
      ),
    })
      .pipe(
        catchError((error) => {
